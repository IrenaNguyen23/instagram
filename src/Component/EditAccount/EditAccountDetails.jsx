...
import ChangeProfilePhotoModal from './ChangProfilePhotoModal'
...
const EditAccountDetails = () => {
        ...
        if (user.reqUser) {
            ...
            for (let item in initialValues) {
                if (user.reqUser[item]) {
                    newValue[item] = user.reqUser[item]; // Sửa từ `user.requser` thành `user.reqUser`
                }
            }
    ...
    }, [user.reqUser, initialValues]); // Sửa từ `user.requser` thành `user.reqUser`

    const formik = useFormik({
        initialValues: initialValues, // Sử dụng chính xác giá trị khởi tạo ban đầu
        enableReinitialize: true, // Cho phép reinitialize khi `initialValues` thay đổi
        onSubmit: (values) => {
            const data = {
                jwt: token,
                data: { ...values, id: user.reqUser?.id }, // Sửa từ `user.requser` thành `user.reqUser`
            };
    ...
    async function handleProfileImageChange(event) {
        ...
        const data = {
            jwt: token,
            data: { image, id: user.reqUser?.id }, // Sửa từ `user.requser` thành `user.reqUser`
        };
        dispatch(editUserAction(data));
            ...
            <div className="flex pb-7">
                <div className="w-[15%]">
                    <img
                        ...
                        src={imageFile || user.reqUser?.image || "https://cdn.pixabay.com/photo/2015/10/05/22/37/blank-profile-picture-973460_640.png"}
                        alt=""
                    />
                </div>
                <div>
